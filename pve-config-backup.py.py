...
def check_nfs_mount():
    if not os.path.ismount(CONFIG["nfs_base_backup_path"]):
        logger.error(f"NFS path {CONFIG['nfs_base_backup_path']} is not mounted")
        return False
    return True
...
def perform_rsync(source, dest):
    ...
    for config_path in source:
        if os.path.exists(config_path):
            try:
                cmd = ["rsync", "-rtDvz", "--relative", "--stats", config_path, backup_path]
...
def main():
    parser = argparse.ArgumentParser(description='Proxmox VE Configuration Backup Service')
    ...
    parser.add_argument('--start', action='store_true', help='Start backup daemon')
    parser.add_argument('--stop', action='store_true', help='Stop backup daemon')
